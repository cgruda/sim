...
Next:	add	$a1,	$a1,	$fp,	0	# pc9	m++
	add	$t0,	$zero,	$sp,	0	# pcA
	blt	$imm,	$a1,	$t0,	mLoop	# pcB	if (m<4) go to mLoop
	add	$zero,	$zero,	$zero,	0	# pcC	(delay slot)
	add	$a0,	$a0,	$fp,	0	# pcD	n++
	add	$t0,	$zero,	$sp,	0	# pcE
	blt	$imm,	$a0,	$t0,	nLoop	# pcF	if (n<4) go to nLoop
	add	$zero,	$zero,	$zero,	0	# pc10	(delay slot)
	lw	$t1,	$imm,	$zero,	0xFF	# ecivt block idx 63
	halt	$zero,	$zero,	$zero,	0	# pc11	n=4, m=4, exit program
	halt	$zero,	$zero,	$zero,	0	# pc12	n=4, m=4, exit program
...
Calc:	add	$t0,	$zero,	$zero,	0	# pc16	reset i
	add	$v0,	$zero,	$zero,	0	# pc17	reset result
iLoop:	mul	$t1,	$t0,	$sp,	0	# pc18	t1=i*16
	add	$t1,	$t1,	$a0,	0	# pc19	t1+=n
	add	$t1,	$t1,	$s0,	0	# pc1A	addr of A(n,i)
	mul	$t2,	$a1,	$sp,	0	# pc1B	t2=m*16
	add	$t2,	$t2,	$t0,	0	# pc1C	t2+=i
	add	$t2,	$t2,	$s1,	0	# pc1D	addr of B(i,m)
	lw	$t1,	$t1,	$zero,	0	# pc1E	A(n,i)=mem[]
	lw	$t2,	$t2,	$zero,	0	# pc1F	B(i,m)=mem[]
	mul	$t1,	$t1,	$t2,	0	# pc20	A(n,i)*B(i,m)
	add	$v0,	$v0,	$t1,	0	# pc21	update result

	add	$t0,	$t0,	$fp,	0	# pc22	i++
	add	$t1,	$zero,	$sp,	0	# pc23	t1=16
	blt	$imm,	$t0,	$t1,	iLoop	# pc24	if (i<16) go to iLoop
	add	$zero,	$zero,	$zero,	0	# pc25	(delay slot)

	mul	$t1,	$a1,	$sp,	0	# pc26	t1=m*16
